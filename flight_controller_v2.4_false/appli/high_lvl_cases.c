...
void HIGH_LVL_Manual(State_drone_t * drone){
	...
	drone->consigne.roll = ((double)(drone->communication.ibus.channels[ROLL]- 1500) / (double)500) * (double)roll_and_pitch_max_setpoint ;
	drone->consigne.pitch = ((double)(drone->communication.ibus.channels[PITCH]- 1500) / (double)500) * (double)roll_and_pitch_max_setpoint ;
	double yaw_consigne_degre_sec = ((double)(drone->communication.ibus.channels[YAW] - 1500) / (double)500) * (double)yaw_max_setpoint ;//Consigne en degré par seconde attention ^^
...
void HIGH_LVL_Manual_Stylee(State_drone_t * drone, State_base_t * base){
	...
	//			---------------------------- 		MAIN PART 			----------------------------------------
	drone->consigne.yaw = ((double)(drone->communication.ibus.channels[YAW] - 1500) / (double)500) * (double)yaw_max_setpoint ;	//Consigne en degré par seconde attention ^^
	drone->consigne.throttle = (double)(drone->communication.ibus.channels[THROTTLE] - 1050)  ;
