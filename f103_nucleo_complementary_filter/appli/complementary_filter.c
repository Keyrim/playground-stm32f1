...
//time variables to find out what the fuck is going on
...
void COMP_FILTER_update_angles(void){
	...
	t1 = SYSTICK_get_time_us();
	COMP_FILTER_angles_e acc_angles ;
	acc_angles.x = (double)raw_data_mpu->Accelerometer_X / acc_sensi ;
	acc_angles.y = (double)raw_data_mpu->Accelerometer_Y / acc_sensi ;
	acc_angles.z = (double)raw_data_mpu->Accelerometer_Z / acc_sensi ;
	t2 = SYSTICK_get_time_us();
	double acc_total = sqrt((acc_angles.x * acc_angles.x ) + (acc_angles.y * acc_angles.y) + (acc_angles.z * acc_angles.z));
	if(acc_total != 0  ){
		if(absolu(acc_angles.x) <= absolu(acc_total))
			acc_angles.x = - asin(acc_angles.x / acc_total ) * (double)57.32;
	...
	//Otherwise, we use the gyro
		...
		COMP_FILTER_angles_e gyro_angles ;
		gyro_angles.x = (double)raw_data_mpu->Gyroscope_X / ((double)frequency * gyro_sensi );
		gyro_angles.y = (double)raw_data_mpu->Gyroscope_Y / ((double)frequency * gyro_sensi );
		gyro_angles.z = (double)raw_data_mpu->Gyroscope_Z / ((double)frequency * gyro_sensi );

		filtered_angles->x += gyro_angles.x - gyro_cal.x ;
		filtered_angles->y += gyro_angles.y - gyro_cal.y ;
		filtered_angles->z += gyro_angles.z - gyro_cal.z ;

		//Pour l'instant en commentaire à rajouter plus tard
		//filtered_angles->x += sin(gyro_angles.z * 0.017) * filtered_angles->y ;
		//filtered_angles->y -= sin(gyro_angles.z * 0.017) * filtered_angles->x ;

		//Complementary filter
		//acc_x used with gyY makes sense dw (it really does btw)
		filtered_angles->x = alpha * filtered_angles->x + (acc_angles.y - acc_cal.y) * ((double)1 - alpha);
		filtered_angles->y = alpha * filtered_angles->y + (acc_angles.x - acc_cal.x) * ((double)1 - alpha);
		//printf("Gx %f\n", filtered_angles->x);

	}
	t3 = SYSTICK_get_time_us();
