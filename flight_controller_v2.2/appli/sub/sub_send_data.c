...
//	}
	...
	if(compteur % angles.periode == 0 || angles.to_send){
		if(compteur_octet >= angles.nb_octet){
			TELEMETRIE_send_angle_x_y_as_int(drone.mpu_angles.x, drone.mpu_angles.y, uart_telem);
	...
	if(compteur % moteurs.periode == 0 || moteurs.to_send ){
		if(compteur_octet >= moteurs.nb_octet){
			TELEMETRIE_send_moteur_all(drone.escs[0].pulsation, drone.escs[1].pulsation, drone.escs[2].pulsation, drone.escs[3].pulsation, uart_telem);
	...
	if(compteur % radio1.periode == 0 || radio1.to_send ){
		if(compteur_octet >= radio1.nb_octet){
			TELEMETRIE_send_channel_all_1_4(drone.channels[0], drone.channels[1], drone.channels[2], drone.channels[3], uart_telem);
	...
	if(compteur % radio2.periode == 0 || radio2.to_send ){
		if(compteur_octet >= radio2.nb_octet){
			TELEMETRIE_send_channel_all_5_8(drone.channels[4], drone.channels[5], drone.channels[6], drone.channels[7], uart_telem);
	...
	if(compteur % batterie.periode == 0 || batterie.to_send ){
		if(compteur_octet >= batterie.nb_octet){
			TELEMETRIE_send_v_bat(drone.v_bat, uart_telem);
	...
	if(compteur % every_is_ok.periode == 0 || every_is_ok.to_send ){
			if(compteur_octet >= every_is_ok.nb_octet){
				TELEMETRIE_send_every_is_ok(((uint8_t)( 2 * drone.ppm_is_ok +  drone.gps_is_ok)), uart_telem);
	...
	if(compteur % state_global.periode == 0 || state_global.to_send ){
		if(compteur_octet >= state_global.nb_octet){
			TELEMETRIE_send_state_flying(drone.state_flight_mode, uart_telem);
